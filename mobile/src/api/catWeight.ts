...
export function useWeightHistory(catId: string) {
	return useQuery({
		...
		queryFn: () => apiGet<WeightHistoryOkResponse>(`/api/cats/${catId}/weight-logs`)
	});
}
...
export function useAddWeightLog(catId: string) {
	...
	return useMutation({
		mutationFn: (input: { weightKg: number; recordedAt: string }) => apiPost(`/api/cats/${catId}/weight-logs`, input),
		onSuccess: () => {
...
export function useDeleteWeightLog(catId: string) {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: (id: string) => apiDelete(`/api/cats/${catId}/weight-logs`, { id }),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['weightHistory', catId] });
		}
	});
